...
params = {
    'dataset': ['synthetic_fedprox'],   # is expected to be one value to construct default experiment name
    'model': ['mclr'],      # is expected to be one value to construct default experiment name
    'num_rounds': [100],
    'eval_every': [1],
    'eval_on_fraction': [1],
...
def run_and_document_experiments(args, experiments_dir, setup_filename, console_output_filename, git_hash):
    ...
    for idx, p in enumerate(parameter_grid):
        ...
        analysator = TangleAnalyser(experiment_folder + '/tangle_data', p['num_rounds'] - 1, experiment_folder + '/tangle_analysis')
