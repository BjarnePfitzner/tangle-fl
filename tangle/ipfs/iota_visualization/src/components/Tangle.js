...
const Node = ({nodeRadius, mouseEntersNodeHandler, mouseLeavesNodeHandler, name, color, client_id, timeCreated}) =>
    <Popup
        trigger={
        ...
        size='mini'>
        <Popup.Header>Transaction {name}</Popup.Header>
        <Popup.Content>
            <ul>
                <li>Peer Id: {client_id}</li>
                <li>Time created: {timeCreated} </li>
            </ul>
        </Popup.Content>
    </Popup>;
                    ...
                    <g transform={`translate(${node.x},${node.y})`} key={node.name}
                       className={
                        ...
                        <Node
                            ...
                            name={node.name}
                            client_id={node.peer.client_id}
                            color={props.selectedPeerId === node.peer.client_id ? '#45eb28' : '#ffffff'}
