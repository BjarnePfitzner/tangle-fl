...
def run_and_document_experiments(args, experiments_dir, setup_filename, console_output_filename):
    ...
    srun_base_command = 'srun -p gpua100,gpupro,gpu --cpus-per-task=16 --mem=128G --job-name=tangle'
    ...
    for idx, p in enumerate(parameter_grid):
        ...
        with open(experiment_folder + '/' + console_output_filename, 'w+') as file:
            ...
            for i in range(start, p['num_rounds'], step):
                ...
                print(f'Command: {srun_base_command} {" ".join(command)}')
                os.system(f'{srun_base_command} {" ".join(command)}')
        ...
        analysator = TangleAnalyser(experiment_folder + '/tangle_data', p['num_rounds'] - 1, experiment_folder + '/tangle_analysis', wandb_id)
