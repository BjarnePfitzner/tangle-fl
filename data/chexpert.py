...
class CheXpertDataset(AbstractDataset):
    ...
    def dataset_size(self):
        total_size = 35144
        train_size = round(total_size * (1 - self.cfg.test_fraction))
        if self.cfg.use_val_data:
            test_size = round(total_size * self.cfg.test_fraction / 2)
            val_size = total_size - (train_size + test_size)
            return {'train': train_size, 'test': test_size, 'val': val_size}
        else:
            test_size = round(total_size * self.cfg.test_fraction)
