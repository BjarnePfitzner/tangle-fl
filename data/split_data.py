...
DATASETS = ['sent140', 'femnist', 'shakespeare', 'celeba', 'synthetic', 'femnist_clustered', 'femnist_clustered_alt', 'goethe']

def create_jsons_for(user_files, which_set, max_users, include_hierarchy, include_cluster_ids):
    ...
    for (i, t) in enumerate(user_files):
        if include_hierarchy and include_cluster_ids:
            (u, h, ns, cid, f) = t
        elif include_hierarchy:
            (u, h, ns, f) = t
        elif include_cluster_ids:
            (u, ns, cid, f) = t
        else:
            (u, ns, f) = t
...
include_cluster_ids = 'cluster_ids' in data
...
if (args.user):
    ...
    for f in files:
        ...
        if include_hierarchy and include_cluster_ids:
            user_files.extend([(u, h, ns, cid, f) for (u, h, ns, cid) in 
                zip(data['users'], data['hierarchies'], data['num_samples'], data['cluster_ids'])])
        elif include_hierarchy:
            user_files.extend([(u, h, ns, f) for (u, h, ns) in 
                zip(data['users'], data['hierarchies'], data['num_samples'])])
        elif include_cluster_ids:
            user_files.extend([(u, ns, cid, f) for (u, ns, cid) in 
                zip(data['users'], data['num_samples'], data['cluster_ids'])])
        else:
    ...
    create_jsons_for(train_user_files, 'train', max_users, include_hierarchy, include_cluster_ids)
    create_jsons_for(test_user_files, 'test', max_users, include_hierarchy, include_cluster_ids)
    ...
    for f in files:
        ...
        cluster_ids = [] # cluster_ids of users that are not deleted
        ...
        for u in users:
            # users array might not be the same as in 'data' object
            # first find the index of the user in the original data and then check its cluster_id at this index
            idx = data['users'].index(u)
            if include_cluster_ids:
                cluster_ids.append(data['cluster_ids'][idx])
        ...
        if include_cluster_ids:
            all_data_train['cluster_ids'] = cluster_ids
            all_data_test['cluster_ids'] = cluster_ids
