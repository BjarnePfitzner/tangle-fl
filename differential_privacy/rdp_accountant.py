...
class RDPAccountant:
    ...
    def get_maximum_n_steps(self, base_n_steps=50, n_steps_increment=10, max_n_steps=None, verbose=False):
        cur_n_steps = base_n_steps
        cur_eps, order = self.get_privacy_spending_for_n_steps(cur_n_steps)
        while cur_eps < self.max_eps:
            if verbose and cur_n_steps % 100 == 0:
                print("Epsilon: {} for {} steps".format(cur_eps, cur_n_steps))
