...
class TipSelector:
    ...
    def future_set(self, tx, approving_transactions, future_set_cache):
        def recurse_future_set(t, max_rec_depth=500):
            if max_rec_depth == 0:
                return future_set_cache
            if t not in future_set_cache:
                direct_approvals = set(approving_transactions[t])
                future_set_cache[t] = direct_approvals.union(*[recurse_future_set(x, max_rec_depth=max_rec_depth-1) for x in direct_approvals])
