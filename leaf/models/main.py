...
def create_clients(users, client_ids, groups, train_data, test_data, model):
    if len(groups) == 0:
        groups = [[] for _ in users]
    clients = [Client(u, cid, g, train_data[u], test_data[u], model) for u, cid, g in zip(users, client_ids, groups)]
    return clients
